...
# ============================================================
    ...
    if not events:
        ...
        high_relevance = sum(
            float(e.get("relevance", 0)) >= 0.75
            for e in events
            if isinstance(e, dict)
        )

        positive_events = sum(
            str(e.get("catalyst", "")).lower() == "positive"
            for e in events
            if isinstance(e, dict)
        )

        negative_events = sum(
            str(e.get("catalyst", "")).lower() == "negative"
            for e in events
            if isinstance(e, dict)
        )
        ...
        if event_labels:
            ...
            catalyst_label = catalyst.upper()

            if catalyst == "positive":
                catalyst_class = "ep-positive"
            elif catalyst == "negative":
                catalyst_class = "ep-negative"
            else:
                catalyst_class = "ep-neutral"

            st.markdown(
                f"""
                <div class="ep-event-detail">
                    <div class="ep-event-detail-top">
                        <span class="ep-event-ticker">
                            {ticker}
                        </span>
                        ...
                        <span class="ep-badge {catalyst_class}">
                            {catalyst_label}
                        </span>
                    </div>

                    <div class="ep-event-headline">
                        {headline}
                    </div>

                    <div class="ep-event-label">
                        EVENT INTELLIGENCE
                    </div>

                    <div class="ep-event-reason">
                        {reason}
                    </div>
                </div>
                """,
            ...
            st.markdown(
                ...
                'EVENT → INVESTMENT COUNCIL'
                '</div>',
                unsafe_allow_html=True,
            )

            handoff_col, info_col = st.columns([1, 2])
            ...
            with info_col:
                st.markdown(
                    f"""
                    <div class="ep-inline-note">
                        <strong>{ticker}</strong>
                        event classified as
                        <strong>{catalyst_label}</strong>
                        with
                        <strong>{relevance:.0%}</strong>
                        relevance.
                        Qwen can evaluate this event against
                        the evidence pack and portfolio context.
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
            ...
            for event in events:
                ...
                if event_catalyst == "POSITIVE":
                    stream_class = "ep-positive"
                elif event_catalyst == "NEGATIVE":
                    stream_class = "ep-negative"
                else:
                    stream_class = "ep-neutral"

                st.markdown(
                    f"""
                    <div class="ep-event-row">
                        <div class="ep-event-row-left">
                            <div class="ep-event-row-ticker">
                                {event_ticker}
                            </div>

                            <div class="ep-event-row-title">
                                {event_headline}
                            </div>

                            <div class="ep-event-row-reason">
                                {event_reason}
                            </div>
                        </div>

                        <div class="ep-event-row-right">
                            <div>
                                {event_relevance:.0%}
                            </div>

                            <span class="ep-badge {stream_class}">
                                {event_catalyst}
                            </span>
                        </div>
                    </div>
