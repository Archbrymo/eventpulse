...
def run_live_qwen_council(event_json, research_json, portfolio_json):
    """Live Qwen council runner used during dashboard testing.

    Intentionally uncached so every refresh makes a fresh council call.
    Errors are returned to the UI instead of being silently swallowed.
    """
    try:
        event = json.loads(event_json) if event_json else {}
        research_pack = json.loads(research_json) if research_json else []
        portfolio = json.loads(portfolio_json) if portfolio_json else {}

        if not event:
            return {
                "_error": "No actionable market event was available for Qwen.",
                "signals": [],
            }

        if not research_pack:
            return {
                "_error": "Research pack is empty; Qwen was not called.",
                "signals": [],
            }
        ...
        # Persist every live Qwen signal.
