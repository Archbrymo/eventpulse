...
live_qwen_error = None
live_qwen_traceback = None

# Always run the real council during testing.
# Pipeline persistence is used only as a fallback if the live council
# cannot produce a decision.
try:
    council_tickers = []

    for row in (research_rows or [])[:10]:
        symbol = row_ticker(row)

        try:
            symbol = underlying_ticker(symbol)
        except Exception:
        ...
        if symbol:
            council_tickers.append(symbol)

    council_tickers = tuple(sorted(set(council_tickers)))

    council_events = []

    if council_tickers:
        council_events = live_events(council_tickers)

    if not council_events:
        council_events = live_events(tuple())

    if council_events and research_rows:
        live_council_event = council_events[0]

        live_council_result = run_live_qwen_council(
            live_council_event,
            research_rows[:10],
            portfolio,
        )

        live_qwen_error = live_council_result.get("_error")
        live_qwen_traceback = live_council_result.get("_traceback")

        if not live_qwen_error:
            pipeline_decisions = live_council_result.get("signals") or []

    elif not council_events:
        live_qwen_error = "The live event feed returned no actionable events."

    elif not research_rows:
        live_qwen_error = "The evidence layer returned no finalists."

except Exception as exc:
    import traceback
    live_qwen_error = f"{type(exc).__name__}: {exc}"
    live_qwen_traceback = traceback.format_exc()


# LIVE QWEN FALLBACK / TEST MODE
# The pipeline may contain no persisted council output yet, so invoke the
# actual council directly from the live research + live event stream.
live_qwen_error = None
live_decision = None

if not pipeline_decisions and research_rows:
    try:
        council_tickers = []
        for row in research_rows:
            symbol = row_ticker(row)
            try:
                council_tickers.append(underlying_ticker(symbol))
            except Exception:
                ...
                if cleaned.endswith("USDT"):
                    cleaned = cleaned[:-4]
                council_tickers.append(cleaned)

        council_tickers = tuple(sorted(set(x for x in council_tickers if x)))

        # First try events associated with the researched finalists.
        council_events = live_events(council_tickers) if council_tickers else []

        # If there are no ticker-specific events, use the broader live event
        # stream rather than silently giving Qwen nothing.
        if not council_events:
            council_events = live_events(tuple())

        if council_events:
            council_event = council_events[0]

            live_decision = run_live_qwen_council(
                json.dumps(council_event, sort_keys=True, default=str),
                json.dumps(research_rows[:10], sort_keys=True, default=str),
                json.dumps(portfolio, sort_keys=True, default=str),
            )

            if live_decision:
                live_qwen_error = live_decision.get("_error")

                if not live_qwen_error:
                    pipeline_decisions = live_decision.get("signals") or []
        else:
            live_qwen_error = "No actionable market event was returned by the live event feed."

    except Exception as exc:
        live_qwen_error = f"{type(exc).__name__}: {exc}"


# If the latest autonomous pipeline did not persist a Qwen result,
# run the actual EventPulse council against the current evidence pack.
if not pipeline_decisions and research_rows:
    try:
        council_events = live_events(
            tuple(
                sorted(
                    set(
                        (
                            ...
                            for row in research_rows
                            if row.get("ticker") or row.get("execution_symbol")
                        )
                    )
                )
            )
        ...
        council_event = council_events[0] if council_events else None
        ...
        if council_event:
            ...
            pipeline_decisions = live_council_signal(
                live_decision
            )
    except Exception as exc:
        pipeline_decisions = []
